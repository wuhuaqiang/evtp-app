...
public class Match {
    ...
    public static double getCurrQuetedPrice(final double min, final double max) { //保留两位小数
        DecimalFormat df = new DecimalFormat("#.00");
        if (min == max) {
    ...
    private static void createBuySel(final int num) {
        QuotedPriceEntity quotedPriceEntity = new QuotedPriceEntity();
        quotedPriceEntity.setId(UUIDKey.getKey());
        quotedPriceEntity.setType(num);
        quotedPriceEntity.setAmount((int) getCurrQuetedPrice(2.0, 25.0));
        quotedPriceEntity.setQuotedPriceTime(currTime);
        quotedPriceEntity.setMark(0);
        if (num == 0) {
            quotedPriceEntity.setPrice(getCurrQuetedPrice(Double.parseDouble(df.format(currElectricityPrice * 0.5)), Double.parseDouble(df.format(currElectricityPrice * 0.95))));
            ...
            quotedPriceEntity.setPrice(getCurrQuetedPrice(Double.parseDouble(df.format(currElectricityPrice * 0.45)), Double.parseDouble(df.format(currElectricityPrice * 0.90))));
            quotedPriceEntity.setCheckPrice(currElectricityPrice);
    ...
    private static void charge(final int num) {
        System.out.println("\n\n\n\n");
