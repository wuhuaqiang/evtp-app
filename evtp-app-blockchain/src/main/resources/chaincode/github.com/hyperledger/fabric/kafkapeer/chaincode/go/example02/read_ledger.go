...
func getElectricityTradingRecordHistory(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	type ElectricityTradingRecordHistory struct {
		TxId  string                   `json:"txId"`
		Value ElectricityTradingRecord `json:"value"`
	}
	var history []ElectricityTradingRecordHistory
	var electricityTradingRecord ElectricityTradingRecord
	var electricityTradingRecordHistoryStr string
	...
	for resultsIterator.HasNext() {
		...
		var tx ElectricityTradingRecordHistory
		tx.TxId = historyData.TxId //copy transaction id over
		fmt.Printf("-historyData:\n%s", historyData)
		fmt.Printf("-TxId:\n%s", historyData.TxId)
		json.Unmarshal(historyData.Value, &electricityTradingRecord) //un stringify it aka JSON.parse()
		fmt.Printf("-Value:\n%s", historyData.Value)
		fmt.Printf("-ElectricityTradingRecord:\n%s", electricityTradingRecord)
		if historyData.Value == nil { //marble has been deleted
			var emptyElectricityTradingRecord ElectricityTradingRecord
			tx.Value = emptyElectricityTradingRecord //copy nil marble
			fmt.Printf("错误")
		} else {
			electricityTradingRecordHistoryStr = string(historyData.Value)
			fmt.Printf("-ElectricityTradingRecordStr:\n%s", electricityTradingRecordHistoryStr)
			json.Unmarshal([]byte(electricityTradingRecordHistoryStr), &electricityTradingRecord)
			fmt.Printf("-ElectricityTradingRecordject:\n%s", electricityTradingRecord)
			//fmt.Println(electricityTradingRecord.OldSoc, ":", electricityTradingRecord.AddSoc, ":", electricityTradingRecord.Money, ":", electricityTradingRecord.BuyerId, ":", electricityTradingRecord.SellerId, electricityTradingRecord.StartTime, ":", electricityTradingRecord.ChargingTime)
			tx.Value = electricityTradingRecord //copy marble over
			//fmt.Printf("-else--ElectricityTradingRecord:\n%s", electricityTradingRecord)
		}
		history = append(history, tx) //add this tx to the list
		fmt.Println("********************************************************")
	}
...
func getQualifiedOfferList(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments. Expecting １")
	}
	qualifiedOfferList_id := args[0] + "ok"
	qualifiedOfferList, err := get_qualifiedOfferList(stub, qualifiedOfferList_id)
	fmt.Printf("-qualifiedOfferList:\n%s", qualifiedOfferList)
	if err != nil {
		return shim.Error(err.Error())
	}
	qualifiedOfferListAsBytes, _ := json.Marshal(qualifiedOfferList) //convert to array of bytes
	return shim.Success(qualifiedOfferListAsBytes)
}
