...
public class SdkInterfaceOrg extends SdkInterfaceBase {
     ...
     * 组织初始化
     */
    public void init() throws InvalidArgumentException, TransactionException, IllegalAccessException, InvocationTargetException, InstantiationException, NoSuchMethodException, CryptoException, ClassNotFoundException, IOException, EnrollmentException, org.hyperledger.fabric_ca.sdk.exception.InvalidArgumentException {
        ...
        String org = "org1";
        String port = "7054";
        if ("Org1MSP".equals(this.orgMSPID)) {
            org = "org1";
            port = "7054";
        } else if ("Org2MSP".equals(this.orgMSPID)) {
            org = "org2";
            port = "8054";
        } else {
            org = "org3";
        ...
        File certificateFile = Paths.get(cryptoConfigPath, "/peerOrganizations/", this.getOrgDomain(), //add by linwf in 2018-07-09
                String.format("/users/%s@%s/msp/signcerts/Admin@" + org + ".example.com-cert.pem", "Admin", this.orgDomain)).toFile(); //add by linwf in 2018-07-09
        ...
        if ("admin".equals(this.username)) {
            ...
            if (StringUtil.isEmpty(this.caName)) {
                hfcaClient = HFCAClient.createNewInstance(this.caLocation, null);
            } else {
                hfcaClient = HFCAClient.createNewInstance(this.caName, this.caLocation, null);
            }
            hfcaClient.setCryptoSuite(CryptoSuite.Factory.getCryptoSuite());
            Enrollment enrollment = hfcaClient.enroll(this.username, this.password);
